...
{
	/// <summary>
	/// Componente <see cref="IExtenderProvider"/> que permite añadir un texto a la 
	/// barra de estado para los elementos de menú y controles de formulario
	/// </summary>
	[ProvideProperty("StatusBarText", typeof(Component)),
	 ProvideProperty("ShowAsBlank", typeof(Component)),
	 Description("Permite definir texto para controles que aparecerá en la barra de estado cuando tengan el foco")]
	...
	{ 
		...
		{	if (disposing)
				{	if (hookedMenuEvents && StatusBar != null)
						{	Form frm = StatusBarParentForm;
		...
		/// <seealso cref="InstanceDisplayPanel"/>
		[Browsable(false),
		 Description("The panel in which to display messages if the application status bar has panels displayed")]
		...
		[Browsable(false),
			DesignerSerializationVisibility(DesignerSerializationVisibility.Hidden),
			Description("The panel to use for the status bar text when not using ApplicationStatusBar")]
...
//               this.CurrentStatusBarText);
		...
		{
				if (StatusBar == null || !htOptions.Contains(sender))
						return;

				CurrentStatusBarText = StatusBarDefaultText;
		}
		...
		public bool CanExtend(object extendee)
		{
				...
				if (!(extendee is Control) || extendee is Form ||
					extendee is System.Windows.Forms.Label || extendee is PictureBox ||
		...
		public string GetStatusBarText(Component comp)
		{
				...
				if (htOptions.Contains(comp))
						return ((StatusBarPropertyOptions)htOptions[comp]).Message;
		...
		public bool GetShowAsBlank(Component comp)
		{
				...
				if (htOptions.Contains(comp))
						return ((StatusBarPropertyOptions)htOptions[comp]).ShowAsBlank;
