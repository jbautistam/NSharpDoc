...
{ /// <summary>
	...
	{ // Enumerados públicos
		...
		// Delegados
			public delegate void ItemDoubleClickHandler(object objSender, string key);
		// Eventos
		...
		{ AddColumn(-2, text, ListViewColumnType.Text);
		}
		
		/// <summary>
		///		Añade la columna ajustando al mayor elemento de texto
		/// </summary>
		public void AddColumn(string text, ListViewColumnType intColumnType)
		{ AddColumn(-2, text, intColumnType);
		}
		
		/// <summary>
		///		Añade una columna
		/// </summary>
		public void AddColumn(int intWidth, string text)
		{ AddColumn(intWidth, text, ListViewColumnType.Text);
		}
		
		/// <summary>
		///		Añade una columna
		/// </summary>
		public void AddColumn(int intWidth, string text, ListViewColumnType intColumnType)
		{ ColumnHeader column = Columns.Add(text, intWidth);
		
				column.Tag = (int) intColumnType;
		...
		///		Añade un elemento
		/// </summary>
		public ListViewItem AddItem(long lngID, string text)
		{ return AddItem(lngID.ToString(), text, 1);
		}
		
		/// <summary>
		///		Añade un elemento
		/// </summary>
		public ListViewItem AddItem(string key, string text)
		{ return AddItem(key, text, 1);
		}
		
		/// <summary>
		///		Añade un elemento
		/// </summary>
		public ListViewItem AddItem(string key, string text, int intImage)
		{ ListViewItem lsiItem = Items.Add(key, text, intImage);
		...
		///		Chequea un elemento
		/// </summary>
		public void CheckItem(string key, bool blnChecked)
		{ ListViewItem lsiItem = Items[key];
		...
		{	// Limpia los grupos
			...
			Dictionary<String, List<ListViewItem>> dctMap = new Dictionary<String, List<ListViewItem>>();
				...
				foreach (ListViewItem lsiItem in Items) 
					{	string key = lsiItem.SubItems[intGroupColumn].Text;
					
							if (intGroupColumn == 0 && key.Length > 0 && GroupByFirstCharacter)
								key = key.Substring(0, 1);
							if (!dctMap.ContainsKey(key))
								dctMap[key] = new List<ListViewItem>();
							dctMap[key].Add(lsiItem);
					}
			// Crea una lista con los grupos necesarios
				List<ListViewGroup> lstGroups = new List<ListViewGroup>();
				
					foreach (string key in dctMap.Keys)
						lstGroups.Add(new ListViewGroup(key));
			// Ordena los grupos
